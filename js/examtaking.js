const examsList = [{"examTitle":"Bài kiểm tra cuối kì","minuteTaking":60,"examNotice":null,"questionList": [{
...
function setPageHeight() {
    ...
    $('.right-side').css('max-height', (windowHeight-navbarHeight*2
        ) + 'px');
    }
...
function generateQuestionByExamId(examId){
    ...
    <h2 class="exam-title">${examItem.examTitle}</h2>
    <h6 class="exam-time">${examItem.minuteTaking} minutes</h6>
    </div>`;
    ...
    examItem.questionList.forEach((questionItem, questionId)=>{
       const examQuestion = `<div id="${
         examId + "-" + questionId
                    ...
                    <h5 class="card-header">Question ${questionId + 1}</h5>
                    <div class="card-body">
    ...
    ${ examItem.examNotice!=null?`<p class="exam-notice">${examItem.examNotice}</p>`:''}
...
function generateTableQuestionByExamId(examId){
    ...
    examItem.questionList.forEach((_, tableQuestionItemIndex)=>{
         document.querySelector(".table-question").insertAdjacentHTML("beforeend",`<a class="${tableQuestionItemIndex===0?"active":""} table-question-item" href="#${examId+'-'+tableQuestionItemIndex}">${tableQuestionItemIndex+1}</a>`)
    })
...
answerListItem.forEach((item) => {
    item.addEventListener('click', () => {
        ...
        const siblingAnserListIetm = parentEachAnwserListItem.querySelectorAll(".answer-card");
        ...
        document.querySelector(".numbers-question").textContent = answeredQuestion+"/"+examsList[0].questionList.length;
    });
});
    ...
    questionListCard.forEach(questionCard => {
        ...
        if (
            ...
            rect.bottom <= (examContent.clientHeight/2 + viewPortRect)
                ...
                tableQuestionListItem.forEach(questionItem=>{
                    if(questionItem.getAttribute("href").substring(1)===(questionCard.getAttribute("id"))){
...
function getAnswersList(){
    ...
    questionListCard.forEach((questionCardItem, questionCardItemIndex)=>{
        questionCardItem.querySelectorAll(".answer-card").forEach(answerCardItem=>{
            if(answerCardItem.classList.contains("active")){
                answersList[questionCardItemIndex] = answerCardItem.querySelector(".answer-card-head").textContent;
            }
        })
    })
    return answersList;
